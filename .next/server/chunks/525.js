"use strict";exports.id=525,exports.ids=[525],exports.modules={85525:(e,t,i)=>{i.d(t,{CP:()=>generateTailoredResume,bG:()=>sendMessage,hN:()=>isApiConfigured,nt:()=>getCurrentModelInfo});let getSettings=()=>{let e=localStorage.getItem("resumeai-settings");return e?JSON.parse(e):{apiKey:"",aiModel:"gpt-4",systemPrompt:"You are an expert resume writer and career advisor. Help users optimize their resumes for ATS systems and specific job descriptions. Provide actionable feedback and suggestions.",temperature:.7,maxTokens:2e3,autoSave:!0}},callOpenAI=async(e,t)=>{let i=await fetch("/api/openai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({apiKey:t.apiKey,model:t.aiModel,messages:[{role:"user",content:t.systemPrompt},...e],temperature:t.temperature,maxTokens:t.maxTokens})});if(!i.ok){let e=await i.json();throw Error(`OpenAI API error: ${e.error?.message||"Unknown error"}`)}let s=await i.json();return s.choices[0].message.content},callClaude=async(e,t)=>{let i={"claude-3-sonnet":"claude-3-5-sonnet-20241022","claude-3-haiku":"claude-3-5-haiku-20241022"}[t.aiModel]||"claude-3-5-sonnet-20241022",s=await fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({apiKey:t.apiKey,model:i,messages:[{role:"user",content:t.systemPrompt},...e],temperature:t.temperature,maxTokens:t.maxTokens})});if(!s.ok){let e=await s.json();throw Error(`Claude API error: ${e.error?.message||"Unknown error"}`)}let n=await s.json();return n.content[0].text},sendMessage=async(e,t=[],i)=>{let s=getSettings();if(!s.apiKey)throw Error("API key not configured. Please set your API key in Settings.");let n=e;i?(n=`Resume Content:
${i}

User Question: ${e}`,console.log(`Sending message with DOCX content (${i.length} chars)`)):console.log("Sending message without DOCX content");let o=[...t,{role:"user",content:n}];return s.aiModel.startsWith("claude")?await callClaude(o,s):await callOpenAI(o,s)},isApiConfigured=()=>{let e=getSettings();return!!e.apiKey},getCurrentModelInfo=()=>{let e=getSettings(),t=e.aiModel.startsWith("claude")?"Anthropic":"OpenAI";return{model:e.aiModel,provider:t}},generateTailoredResume=async(e,t)=>{let i;let s=getSettings();if(!s.apiKey)throw Error("API key not configured. Please set your API key in Settings.");let n=`You are an expert resume writer specializing in LaTeX resume creation. Your task is to generate a tailored resume in LaTeX format based on a job description and optionally the user's existing resume content.
...
${t?`Existing Resume Content:
${t}`:"No existing resume content provided - create a complete professional resume based on the job description."}
...
Generate the response in the exact format specified above.`,o=[{role:"user",content:n}];i=s.aiModel.startsWith("claude")?await callClaude(o,s):await callOpenAI(o,s);let a=i.match(/===LATEX_RESUME===\s*([\s\S]*?)\s*===IMPROVEMENTS===/),r=i.match(/===IMPROVEMENTS===\s*([\s\S]*?)$/),l=a?a[1].trim():i,c=r?r[1].trim():"No specific improvements identified.";return{resume:l,improvements:c}}}};